-- Participant auth-identity reconciliation fix for seat claims
--
-- Problem:
--   public.participants is globally UNIQUE on BOTH client_id and auth_user_id
--   (idx_participants_auth_user_id). The previous claim_session_role_seat upsert
--   only reconciled `ON CONFLICT (client_id)`. When a stable browser identity
--   (auth.uid()) arrived with a NEW client_id -- e.g. cleared/regenerated
--   esg_client_id, a different tab, or a leftover participants row from an
--   earlier session -- the INSERT did not hit the client_id conflict and instead
--   collided on auth_user_id, failing with:
--       duplicate key value violates unique constraint "idx_participants_auth_user_id"
--
-- Fix:
--   Treat auth_user_id as the authoritative participant identity. Look the
--   participant up by auth_user_id first and reuse that row (refreshing its
--   client_id/name/role). Only INSERT when no row exists for the identity, still
--   reconciling a stray client_id row in that path. Everything else about the
--   seat-claim contract is unchanged.
--
-- Safe to reapply (CREATE OR REPLACE).
...
BEGIN
    ...
    -- Reconcile the participant on the authoritative auth identity first.
    -- participants is globally unique on BOTH client_id and auth_user_id, so a
    -- stable browser identity that arrives with a new client_id must reuse its
    -- existing row instead of inserting a duplicate (which previously failed on
    -- idx_participants_auth_user_id).
    SELECT *
    ...
    IF FOUND THEN
        -- Release the requested client_id if any other (stale) row still holds
        -- it; the session_participants FK is ON DELETE CASCADE, so any stale
        -- seat attached to that duplicate is cleaned up with it.
